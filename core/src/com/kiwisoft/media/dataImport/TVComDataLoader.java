 ...
 */
public abstract class TVComDataLoader implements Job
{
	public static final SimpleDateFormat AIRDATE_FORMAT=new SimpleDateFormat("M/d/yyyy");
	...
	protected TVComDataLoader(Show show, String baseUrl, int startSeason, int endSeason, boolean autoCreate)
	{
		this.show=show;
...
//		FileUtils.saveToFile(page, new File("c:"+File.separator+"Incoming"+File.separator+"season"+season+".html"));
		...
		while (!progress.isStoppedByUser())
		{
			...
			try
			{
				airdate=AIRDATE_FORMAT.parse(values.get(2));
...
//		FileUtils.saveToFile(episodePage, new File("c:"+File.separator+"Incoming"+File.separator+"episode"+data.getEpisodeNumber()+".html"));
		...
		applyEpisode(episode, episodeData);
	}
	...
	public void applyEpisode(final Episode episode, final EpisodeData data)
	{
