...
//			File[] files=new File("c:\\Incoming\\tvtv").listFiles();
	...
	private void loadShowDates(Show show, String searchUrl, String patternString) throws IOException
	{
	...
	private void parseDetails(String content, String id) throws IOException
	{
		...
		System.out.println("day = "+dateString);
		...
		String show=content.substring(index+1, index2);
		show=XMLUtils.resolveEntities(XMLUtils.removeTags(show)).trim();
		...
		if (index>=lastIndex)
		{
			...
			if (otitle!=null) System.out.println("otitle = "+otitle);
			lastIndex=index;
		}
...
//			lastIndex=index;
		...
		String fileName=path+File.separator+URLEncoder.encode(show, "UTF-8");
		...
		fw.write("<Show>"+XMLUtils.toXMLString(show)+"</Show>\n");
	...
	private void saveFile(String content, String fileName) throws IOException
	{
