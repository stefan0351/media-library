/*
    ...
    */
package com.kiwisoft.web.struts;
...
/**
 * An Interceptor to preserve an actions ValidationAware messages
 * across a
 * redirect result.
 * <p/>
 * It makes the assumption that you always want to preserve messages
 * across a
 * redirect and restore them to the next action if they exist.
 * <p/>
 * The way this works is it looks at the result type after a action
 * has executed
 * and if the result was a redirect (ServletRedirectResult) or a
 * redirectAction
 * (ServletActionRedirectResult) and there were any errors, messages, or
 * fieldErrors they are stored in the session. Before the next
 * action executes
 * it will check if there are any messages stored in the session and
 * add them to
 * the next action.
 */
public class RedirectMessageInterceptor extends MethodFilterInterceptor
{
	...
	/**
	 * Retrieve the errors and messages from the session and add
	 * them to the
	 * action.
	 */
	...
	/**
	 * If the result is a redirect then store error and messages in
	 * the session.
	 */
